<?php
    ...
    }$_SESSION['user_l'] = htmlspecialchars(addslashes(trim($_GET['name'])));
    $_SESSION['id_user_l'] = htmlspecialchars(addslashes(trim($_GET['id'])));
?>
<!DOCTYPE HTML>
<html>
    <head>
    ...
    <div>
        ...
        <div class="container">
            <div class="row">
                <div class="col">
                    <h2>Usuario: <?=$_SESSION['user_l']?></h2>
                </div>
            </div>
        </div><?php
            $_user_id = $_SESSION['id_user_l'];
        ...
        <div class="container-fluid">
            <div class="row-auto">
                ...
                $_SESSION['id_l'] = $id;
                $consulta = "SELECT id_notas, nombre_n FROM notas WHERE Usuarios_id_usuario = $id";
                $query =  $conexion->query($consulta);
                    if($query){    
                        while($datos = $query->fetch_assoc()){
                        $datosE['id_notas'] = $datos['id_notas'];
                        $datosE['nombre'] = $datos['nombre_n'];
                        $_SESSION['id_notas_l'] = $datosE['id_notas'];
                        $_id_notes_l = $_SESSION['id_notas_l'];
                        ?><tr>
                        <td><?=$datosE['id_notas']?></td>
                            <td><?=$datosE['nombre']?></td>
                            <td>
                                    <a href="notes_delete.php?id=<?=$_SESSION['id_notas_l']?>" role="button" onclick="return confirm('¿Está seguro de eliminar el registro?')">
                                    <img src="../../../Base_de_datos/images-icons/deleted.png" 
                                        width="25" alt="edit_note"/></a>
                            </td>
                            <td>
                                <a href="v_notes_user.php?&user=<?=$_id_notes_l?>">
                    ...
                    <div class="container bg-light m-5 border rounded-top">
                        <div class="row justify-content-center">
                            <div class="caja m-5 w-100 bg-light">
                                <div class="col">
                                    <h1 class="h1 display-1" align="center">¡Error al recuperar los registros!</h1>
                                </div>
                            </div>
                        </div>
                    </div><?php
        ...
        <div class="container-fluid m-3">
                <div class="row-auto">
                    <div class="col-auto">
                        <a href="../admin_index.php" role="button" class="btn btn-warning">Atras</a>
                    </div>
                </div>
            </div>
    </body>
